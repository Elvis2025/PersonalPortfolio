export type ContactInput = { name?: string; email?: string; subject?: string; message?: string; company?: string; language?: 'en' | 'es' };
...
export function validateContact(input: ContactInput): ContactMessage {
  ...
  return { name: input.name.trim(), email, subject: input.subject.trim(), message: input.message.trim(), language: input.language === 'es' ? 'es' : 'en' };
}
